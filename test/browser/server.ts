		...
		<script type="module">
			...
			const server = new ChannelServer({
				...
				handler: {
					...
					fail: () => { throw new Error('private failure'); }
				}
			});
			server.start();
...
const childPage = `<!doctype html><html lang="en"><head><title>Channel RPC child</title></head><body>
	...
	<script type="module">
		...
		const client = new ChannelClient({ target: window.parent, channelId: 'browser-test', timeout: mode === 'deny' ? 500 : 2000 });
...
function fetch(request: Request) {
	...
	if (url.pathname === '/parent.html') {
		...
		if (!['success', 'errors', 'deny'].includes(mode)) return new Response('Unknown mode', { status: 400 });
